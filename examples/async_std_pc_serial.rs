...
async fn main() -> Result<(), AppError> {
    ...
    loop {
        if connector.stop_multiple_polling_instructions().await.is_err() {
    ...
    info!(
        "Working area: {:?}",
        connector
            .get_working_area().await
            .map_err(|e| AppError::Connector(e.to_string()))?
    );
    info!(
        "Working channel: {:?}",
        connector
            .get_working_channel().await
            .map_err(|e| AppError::Connector(e.to_string()))?
    );

    let transmission_power = connector
        .get_transmit_power().await
        .map_err(|e| AppError::Connector(e.to_string()))?;
    ...
    if transmission_power != power {
        info!(
            ...
            connector
                .set_transmission_power(power).await
                .map_err(|e| AppError::Connector(e.to_string()))?
        );
    }
    ...
    for sequence in 0..10 {
        for i in connector
            .multi_polling_instruction().await
            .map_err(|e| AppError::Connector(e.to_string()))?
        {
